...
var (
	...
	skipNonceDestinationAddress = map[string]bool{
		common.GelXAddr:                         true,
		common.TradingStateAddr:                  true,
		common.GelXLendingAddress:               true,
		common.GelXLendingFinalizedTradeAddress: true,
	}
)
...
func (tx *Transaction) AsMessage(s Signer, balanceFee *big.Int, number *big.Int) (Message, error) {
	...
	if balanceFee != nil {
		if number.Cmp(common.TIPGRC21Fee) > 0 {
			msg.gasPrice = common.GRC21GasPrice
		} else {
			msg.gasPrice = common.GRC21GasPriceBefore
		}
	}
	return msg, err
}
...
// Cost returns amount + gasprice * gaslimit.
func (tx *Transaction) GRC21Cost() *big.Int {
	total := new(big.Int).Mul(common.GRC21GasPrice, new(big.Int).SetUint64(tx.data.GasLimit))
	total.Add(total, tx.data.Amount)
	return total
}
...
func (tx *Transaction) IsTradingTransaction() bool {
	...
	if tx.To().String() != common.GelXAddr {
		return false
	}
...
func (tx *Transaction) IsLendingTransaction() bool {
	...
	if tx.To().String() != common.GelXLendingAddress {
		return false
	}
	return true
}
...
func (tx *Transaction) IsLendingFinalizedTradeTransaction() bool {
	...
	if tx.To().String() != common.GelXLendingFinalizedTradeAddress {
		return false
	}
	return true
}
...
func (tx *Transaction) IsGelXApplyTransaction() bool {
	if tx.To() == nil {
		return false
	}

	addr := common.GelXListingSMC
	if common.IsTestnet {
		addr = common.GelXListingSMCTestNet
	}
	...
	if method != common.GelXApplyMethod {
		return false
	}
...
func (tx *Transaction) IsGelaZApplyTransaction() bool {
	if tx.To() == nil {
		return false
	}

	addr := common.GRC21IssuerSMC
	if common.IsTestnet {
		addr = common.GRC21IssuerSMCTestNet
	}
	if tx.To().String() != addr.String() {
		return false
	}
	...
	method := common.ToHex(tx.Data()[0:4])
	if method != common.GelaZApplyMethod {
		return false
	}
