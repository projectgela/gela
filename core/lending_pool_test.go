...
func getLendingNonce(userAddress common.Address) (uint64, error) {
	...
	err = rpcClient.Call(&result, "gelx_getLendingOrderCount", userAddress)
	if err != nil {
		return 0, err
	}
	s := result.(string)
...
func TestSendLending(t *testing.T) {
	...
	for true {
		...
		// lendToken: USD, collateral: GEL
		// amount 1000 USD
		testSendLending(key, nonce, USDAddress, common.Address{}, new(big.Int).Mul(_1E8, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, USDAddress, common.HexToAddress(common.GelaNativeAddress), new(big.Int).Mul(_1E8, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)

		// lendToken: BTC, collateral: GEL
		// amount 1 BTC
		...
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, BTCAddress, common.HexToAddress(common.GelaNativeAddress), new(big.Int).Mul(_1E18, big.NewInt(1)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		...
		// lendToken: GEL, collateral: BTC
		// amount 1000 GEL
		testSendLending(key, nonce, common.HexToAddress(common.GelaNativeAddress), common.Address{}, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, common.HexToAddress(common.GelaNativeAddress), BTCAddress, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)

		// lendToken: GEL, collateral: ETH
		// amount 1000 GEL
		testSendLending(key, nonce, common.HexToAddress(common.GelaNativeAddress), common.Address{}, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Investing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
		testSendLending(key, nonce, common.HexToAddress(common.GelaNativeAddress), ETHAddress, new(big.Int).Mul(_1E18, big.NewInt(1000)), interestRate, lendingstate.Borrowing, lendingstate.LendingStatusNew, true, 0, 0, common.Hash{}, "")
		nonce++
		time.Sleep(time.Second)
	}
}
