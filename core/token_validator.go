...
func CallContractWithState(call ethereum.CallMsg, chain consensus.ChainContext, statedb *state.StateDB) ([]byte, error) {
	...
	// Execute the call.
	msg := callmsg{call}
	feeCapacity := state.GetGRC21FeeCapacityFromState(statedb)
	if msg.To() != nil {
...
func ValidateGelXApplyTransaction(chain consensus.ChainContext, blockNumber *big.Int, copyState *state.StateDB, tokenAddr common.Address) error {
	if blockNumber == nil || blockNumber.Sign() <= 0 {
		blockNumber = chain.CurrentHeader().Number
	}
	if !chain.Config().IsTIPGelX(blockNumber) {
		return nil
	}
	contractABI, err := GetTokenAbi(contract.GRC21ABI)
	if err != nil {
		return fmt.Errorf("ValidateGelXApplyTransaction: cannot parse ABI. Err: %v", err)
	}
...
func ValidateGelaZApplyTransaction(chain consensus.ChainContext, blockNumber *big.Int, copyState *state.StateDB, tokenAddr common.Address) error {
	if blockNumber == nil || blockNumber.Sign() <= 0 {
		blockNumber = chain.CurrentHeader().Number
	}
	if !chain.Config().IsTIPGelX(blockNumber) {
	...
	contractABI, err := GetTokenAbi(contract.GRC21ABI)
	if err != nil {
		return fmt.Errorf("ValidateGelaZApplyTransaction: cannot parse ABI. Err: %v", err)
	}
	// verify balance slot
	if err := ValidateBalanceSlot(chain, copyState, tokenAddr, contractABI); err != nil {
		return err
...
func SetRandomBalance(copyState *state.StateDB, tokenAddr, addr common.Address, randomValue *big.Int) {
	slotBalanceGrc21 := state.SlotGRC21Token["balances"]
	balanceKey := state.GetLocMappingAtKey(addr.Hash(), slotBalanceGrc21)
...
func ValidateBalanceSlot(chain consensus.ChainContext, copyState *state.StateDB, tokenAddr common.Address, contractABI *abi.ABI) error {
	...
	if err != nil || result == nil {
		return fmt.Errorf("cannot get balance at slot %v . Token: %s . Err: %v", state.SlotGRC21Token["balances"], tokenAddr.Hex(), err)
	}
	balance, ok := result.(*big.Int)
	if !ok {
		return fmt.Errorf("invalid balance at slot %v . Token: %s . GotBalance: %v . ResultType: %T", state.SlotGRC21Token["balances"], tokenAddr.Hex(), result, result)
	}
...
func ValidateMinFeeSlot(chain consensus.ChainContext, copyState *state.StateDB, tokenAddr common.Address, contractABI *abi.ABI) error {
	...
	slotMinFeeGrc21 := state.SlotGRC21Token["minFee"]
	copyState.SetState(tokenAddr, common.BigToHash(new(big.Int).SetUint64(slotMinFeeGrc21)), common.BytesToHash(randomValue.Bytes()))

	result, err := RunContract(chain, copyState, tokenAddr, contractABI, minFeeFunction)
	if err != nil || result == nil {
		return fmt.Errorf("cannot get minFee at slot %v . Token: %s. Err: %v", state.SlotGRC21Token["minFee"], tokenAddr.Hex(), err)
	}
	minFee, ok := result.(*big.Int)
	if !ok {
		return fmt.Errorf("invalid minFee at slot %v . Token: %s . GotMinFee: %v . ResultType: %T", state.SlotGRC21Token["minFee"], tokenAddr.Hex(), result, result)
	}
