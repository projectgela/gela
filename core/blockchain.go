...
func NewBlockChainEx(db ethdb.Database, gelxDb ethdb.GelxDatabase, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	blockchain, err := NewBlockChain(db, cacheConfig, chainConfig, engine, vmConfig)
	if err != nil {
		return nil, err
	}
	if blockchain != nil {
		blockchain.addGelxDb(gelxDb)
	}
...
func (bc *BlockChain) addGelxDb(gelxDb ethdb.GelxDatabase) {
	bc.gelxDb = gelxDb
}
...
func (bc *BlockChain) loadLastState() error {
	...
	if err != nil {
		...
		if ok {
			tradingService := engine.GetGelXService()
			lendingService := engine.GetLendingService()
			if bc.Config().IsTIPGelX(currentBlock.Number()) && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) OrderStateAt(block *types.Block) (*tradingstate.TradingStateDB, error) {
	engine, ok := bc.Engine().(*posv.Posv)
	if ok {
		gelXService := engine.GetGelXService()
		if bc.Config().IsTIPGelX(block.Number()) && gelXService != nil {
			...
			if err == nil {
				return gelxState, nil
			} else {
				return nil, err
			}
		}
	}
	return nil, errors.New("Get gelx state fail")
...
func (bc *BlockChain) LendingStateAt(block *types.Block) (*lendingstate.LendingStateDB, error) {
	...
	if ok {
		lendingService := engine.GetLendingService()
		if bc.Config().IsTIPGelX(block.Number()) && lendingService != nil {
			author, _ := bc.Engine().Author(block.Header())
		...
		if (common.Rewound == uint64(0)) || ((*head).Number().Uint64() < common.Rewound) {
			if _, err := state.New((*head).Root(), bc.stateCache); err == nil {
				...
				if ok {
					tradingService := engine.GetGelXService()
					lendingService := engine.GetLendingService()
					if bc.Config().IsTIPGelX((*head).Number()) && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) HasFullState(block *types.Block) bool {
	...
	if bc.Config().IsTIPGelX(block.Number()) && engine != nil && block.NumberU64() > bc.chainConfig.Posv.Epoch {
		tradingService := engine.GetGelXService()
		lendingService := engine.GetLendingService()
		author, _ := bc.Engine().Author(block.Header())
...
func (bc *BlockChain) SaveData() {
	...
	if !bc.cacheConfig.Disabled {
		...
		if bc.Config().IsTIPGelX(bc.CurrentBlock().Number()) && engine != nil {
			if tradingService := engine.GetGelXService(); tradingService != nil && tradingService.GetStateCache() != nil {
		...
		for _, offset := range []uint64{0, 1, triesInMemory - 1} {
			if number := bc.CurrentBlock().NumberU64(); number > offset {
				...
				if bc.Config().IsTIPGelX(bc.CurrentBlock().Number()) && engine != nil {
					...
					if tradingService := engine.GetGelXService(); tradingService != nil {
		...
		if bc.Config().IsTIPGelX(bc.CurrentBlock().Number()) && engine != nil && tradingTriedb != nil && lendingTriedb != nil {
			if tradingService := engine.GetGelXService(); tradingService != nil && tradingService.GetTriegc() != nil {
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, tradingState *tradingstate.TradingStateDB, lendingState *lendingstate.LendingStateDB) (status WriteStatus, err error) {
	...
	engine, _ := bc.Engine().(*posv.Posv)
	var tradingTrieDb *trie.Database
	if bc.Config().IsTIPGelX(block.Number()) && engine != nil {
		if tradingService := engine.GetGelXService(); tradingService != nil {
			tradingTrieDb = tradingService.GetStateCache().TrieDB()
		}
	}
	...
	var lendingTrieDb *trie.Database
	if bc.Config().IsTIPGelX(block.Number()) && engine != nil {
		if lendingService := engine.GetGelXService(); lendingService != nil {
			lendingTrieDb = lendingService.GetStateCache().TrieDB()
		}
	}
	triedb := bc.stateCache.TrieDB()
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if size > limit || bc.gcproc > bc.cacheConfig.TrieTimeLimit || chosen > lastWrite+triesInMemory {
				...
				if chosen >= lastWrite+triesInMemory || size >= 2*limit || bc.gcproc >= 2*bc.cacheConfig.TrieTimeLimit {
					...
					lastWrite = chosen
					bc.gcproc = 0
					if bc.Config().IsTIPGelX(block.Number()) && tradingTrieDb != nil && lendingTrieDb != nil {
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
			...
			if tradingService != nil && lendingService != nil {
				...
				if tradingState != nil && tradingService != nil {
					...
					if gotRoot != expectRoot {
						err = fmt.Errorf("invalid gelx trading state merke trie got : %s , expect : %s ,parent : %s", gotRoot.Hex(), expectRoot.Hex(), parentRoot.Hex())
						bc.reportBlock(block, nil, err)
						return i, events, coalescedLogs, err
					}
					log.Debug("GelX Trading State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
				if lendingState != nil && tradingState != nil {
					...
					log.Debug("GelX Lending State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
			}
		}
		feeCapacity := state.GetGRC21FeeCapacityFromStateWithCache(parent.Root(), statedb)
		...
		case CanonStatTy:
			...
			if bc.chainConfig.IsTIPGelX(block.Number()) {
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if bc.chainConfig.IsTIPGelX(commonBlock.Number()) {
...
func (bc *BlockChain) logExchangeData(block *types.Block) {
	...
	gelXService := engine.GetGelXService()
	if gelXService == nil || !gelXService.IsSDKNode() {
		return
	}
	txMatchBatchData, err := ExtractTradingTransactions(block.Transactions())
	if err != nil {
	...
	for _, txMatchBatch := range txMatchBatchData {
		...
		for _, txMatch := range txMatchBatch.Data {
			...
			if err := gelXService.SyncDataToSDKNode(takerOrderInTx, txMatchBatch.TxHash, txMatchTime, currentState, trades, rejectedOrders, &dirtyOrderCount); err != nil {
...
func (bc *BlockChain) reorgTxMatches(deletedTxs types.Transactions, newChain types.Blocks) {
	...
	for _, deletedTx := range deletedTxs {
		if deletedTx.IsTradingTransaction() {
			...
			if err := gelXService.RollbackReorgTxMatch(deletedTx.Hash()); err != nil {
