...
type OrderPool struct {
	...
	pendingState      *tradingstate.GelXManagedState // Pending state tracking virtual nonces
...
func NewOrderPool(chainconfig *params.ChainConfig, chain blockChainGelx) *OrderPool {
...
func (pool *OrderPool) State() *tradingstate.GelXManagedState {
	pool.mu.RLock()
...
func (pool *OrderPool) validateOrder(tx *types.OrderTransaction) error {
	...
	cloneGelXStateDb := pool.currentOrderState.Copy()
	...
	if !tx.IsCancelledOrder() {
		...
		if orderType == OrderTypeLimit {
			...
			baseDecimal, err := gelXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.BaseToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get baseDecimal. err: %v", err)
			}
			quoteDecimal, err := gelXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.QuoteToken())
			if err != nil {
				return fmt.Errorf("validateOrder: failed to get quoteDecimal. err: %v", err)
			}
			if err := tradingstate.VerifyBalance(cloneStateDb, cloneGelXStateDb, tx, baseDecimal, quoteDecimal); err != nil {
				return err
			}
		}
	...
	if !tx.IsCancelledOrder() {
		...
		originOrder := cloneGelXStateDb.GetOrder(tradingstate.GetTradingOrderBookHash(tx.BaseToken(), tx.QuoteToken()), common.BigToHash(new(big.Int).SetUint64(tx.OrderID())))
