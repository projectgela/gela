...
func (b *BlockGen) AddTxWithChain(bc *BlockChain, tx *types.Transaction) {
	...
	feeCapacity := state.GetGRC21FeeCapacityFromState(b.statedb)
	...
	if tokenFeeUsed {
		...
		if b.header.Number.Cmp(common.TIPGRC21Fee) > 0 {
			fee = fee.Mul(fee, common.GRC21GasPrice)
		}
		state.UpdateGRC21Fee(b.statedb, map[common.Address]*big.Int{*tx.To(): new(big.Int).Sub(feeCapacity[*tx.To()], new(big.Int).SetUint64(gas))}, fee)
	}
}
