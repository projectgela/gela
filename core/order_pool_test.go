...
func getNonce(t *testing.T, userAddress common.Address) (uint64, error) {
	...
	err = rpcClient.Call(&result, "gelx_getOrderCount", userAddress)
	if err != nil {
		return 0, err
	}
	s := result.(string)
...
func testSendOrder(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
	...
	msg := &OrderMsg{
		...
		BaseToken:       common.HexToAddress(common.GelaNativeAddress),
...
func testSendOrderGELAUSD(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func testSendOrderGELABTC(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func TestFilled(t *testing.T) {
	...
	//testSendOrderGELAUSD(t, new(big.Int).Mul(big.NewInt(1000000000000000000), big.NewInt(5000)), BTCUSDPrice, "BUY", "NEW", 0)
	//ETH/BTC
	...
	GELABTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	time.Sleep(2 * time.Second)
	testSendOrderGELABTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), GELABTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderGELABTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), GELABTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderGELABTC(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), GELABTCPrice, "SELL", "NEW", 0)

	GELAUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(6)) // 0.6
	time.Sleep(2 * time.Second)
	testSendOrderGELAUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), GELAUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderGELAUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), GELAUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderGELAUSD(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), GELAUSDPrice, "SELL", "NEW", 0)

}

func TestX10Filled(t *testing.T) {
	GELAUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(60)) // 6
	time.Sleep(2 * time.Second)
	testSendOrderGELAUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), GELAUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderGELAUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), GELAUSDPrice, "BUY", "NEW", 0)
...
func TestCancelOrder(t *testing.T) {
	GELABTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), GELABTCPrice, "BUY", "NEW", 0)
	time.Sleep(5 * time.Second)
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), GELABTCPrice, "BUY", "CANCELLED", 3)
	time.Sleep(5 * time.Second)
