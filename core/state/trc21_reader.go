...
var (
	SlotGRC21Issuer = map[string]uint64{
		"minCap":      0,
		"tokens":      1,
		"tokensState": 2,
	}
	SlotGRC21Token = map[string]uint64{
		"balances": 0,
		"minFee":   1,
		"issuer":   2,
	}
...
func GetGRC21FeeCapacityFromStateWithCache(trieRoot common.Hash, statedb *StateDB) map[common.Address]*big.Int {
	if statedb == nil {
	...
	if data != nil {
		...
		info = GetGRC21FeeCapacityFromState(statedb)
	}
...
func GetGRC21FeeCapacityFromState(statedb *StateDB) map[common.Address]*big.Int {
	if statedb == nil {
		return map[common.Address]*big.Int{}
	}
	tokensCapacity := map[common.Address]*big.Int{}
	slotTokens := SlotGRC21Issuer["tokens"]
	slotTokensHash := common.BigToHash(new(big.Int).SetUint64(slotTokens))
	slotTokensState := SlotGRC21Issuer["tokensState"]
	tokenCount := statedb.GetState(common.GRC21IssuerSMC, slotTokensHash).Big().Uint64()
	for i := uint64(0); i < tokenCount; i++ {
		key := GetLocDynamicArrAtElement(slotTokensHash, i, 1)
		value := statedb.GetState(common.GRC21IssuerSMC, key)
		if !common.EmptyHash(value) {
			token := common.BytesToAddress(value.Bytes())
			balanceKey := GetLocMappingAtKey(token.Hash(), slotTokensState)
			balanceHash := statedb.GetState(common.GRC21IssuerSMC, common.BigToHash(balanceKey))
...
func PayFeeWithGRC21TxFail(statedb *StateDB, from common.Address, token common.Address) {
	if statedb == nil {
		return
	}
	slotBalanceGrc21 := SlotGRC21Token["balances"]
	balanceKey := GetLocMappingAtKey(from.Hash(), slotBalanceGrc21)
	...
	if !common.EmptyHash(balanceHash) {
		...
		issuerTokenKey := GetLocSimpleVariable(SlotGRC21Token["issuer"])
		if common.EmptyHash(issuerTokenKey) {
			return
		}
		issuerAddr := common.BytesToAddress(statedb.GetState(token, issuerTokenKey).Bytes())
		feeTokenKey := GetLocSimpleVariable(SlotGRC21Token["minFee"])
		...
		issuerBalanceKey := GetLocMappingAtKey(issuerAddr.Hash(), slotBalanceGrc21)
...
func ValidateGRC21Tx(statedb *StateDB, from common.Address, token common.Address, data []byte) bool {
	...
	if !common.EmptyHash(balanceHash) {
		balance := balanceHash.Big()
		minFeeTokenKey := GetLocSimpleVariable(SlotGRC21Token["minFee"])
...
func UpdateGRC21Fee(statedb *StateDB, newBalance map[common.Address]*big.Int, totalFeeUsed *big.Int) {
	...
	for token, value := range newBalance {
		balanceKey := GetLocMappingAtKey(token.Hash(), slotTokensState)
		statedb.SetState(common.GRC21IssuerSMC, common.BigToHash(balanceKey), common.BigToHash(value))
	}
	statedb.SubBalance(common.GRC21IssuerSMC, totalFeeUsed)
}
