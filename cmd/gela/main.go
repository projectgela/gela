...
const (
	clientIdentifier = "gela" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the gela command line interface")
	...
	nodeFlags = []cli.Flag{
		...
		utils.GelXEnabledFlag,
		utils.GelXDataDirFlag,
		utils.GelXDBEngineFlag,
		utils.GelXDBConnectionUrlFlag,
		utils.GelXDBReplicaSetNameFlag,
		utils.GelXDBNameFlag,
...
func init() {
	// Initialize the CLI app and start gela
	app.Action = gela
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright (c) 2018 Gela"
...
// gela is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gela(ctx *cli.Context) error {
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, cfg gelaConfig) {
	...
	if _, ok := ethereum.Engine().(*posv.Posv); ok {
		go func() {
			started := false
			ok := false
			slaveMode := ctx.GlobalIsSet(utils.GelaSlaveModeFlag.Name)
			var err error
			if common.IsTestnet {
