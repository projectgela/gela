...
func loadConfig(file string, cfg *gelaConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gela.ipc"
	return cfg
}

func makeConfigNode(ctx *cli.Context) (*node.Node, gelaConfig) {
	// Load defaults.
	cfg := gelaConfig{
		Eth:         eth.DefaultConfig,
		Shh:         whisper.DefaultConfig,
		GelX:       gelx.DefaultConfig,
	...
	if ctx.GlobalBool(utils.GelaTestnetFlag.Name) {
		common.IsTestnet = true
		common.GRC21IssuerSMC = common.GRC21IssuerSMCTestNet
		cfg.Eth.NetworkId = 89
		common.RelayerRegistrationSMC = common.RelayerRegistrationSMCTestnet
		common.TIPGRC21Fee = common.TIPGelXTestnet
	}
	...
	utils.SetGelXConfig(ctx, &cfg.GelX, cfg.Node.DataDir)
	return stack, cfg
}
...
func makeFullNode(ctx *cli.Context) (*node.Node, gelaConfig) {
	stack, cfg := makeConfigNode(ctx)

	// Register GelX's OrderBook service if requested.
	// enable in default
	utils.RegisterGelXService(stack, &cfg.GelX)
