...
var (
	...
	minApply = big.NewInt(0).Mul(big.NewInt(1000), big.NewInt(100000000000000000)) // 100 GEL
)

func TestFeeTxWithGRC21Token(t *testing.T) {
	...
	grc21IssuerAddr, grc21Issuer, err := DeployGRC21Issuer(transactOpts, contractBackend, minApply)

	//set contract address to config
	common.GRC21IssuerSMC = grc21IssuerAddr
	if err != nil {
		t.Fatal("can't deploy smart contract: ", err)
	}
	contractBackend.Commit()
	cap := big.NewInt(0).Mul(big.NewInt(10000000), big.NewInt(10000000000000))
	GRC21fee := big.NewInt(100)
	//  deploy a GRC21 SMC
	grc21TokenAddr, grc21, err := DeployGRC21(transactOpts, contractBackend, "TEST", "GEL", 18, cap, GRC21fee)
	if err != nil {
		t.Fatal("can't deploy smart contract: ", err)
	}
	contractBackend.Commit()
	// add grc21 address to list token grc21Issuer
	grc21Issuer.TransactOpts.Value = minApply
	...
	balance, err := contractBackend.BalanceAt(nil, grc21IssuerAddr, nil)
	if err != nil || balance.Cmp(minApply) != 0 {
		t.Fatal("can't get balance  in grc21Issuer SMC: ", err, "got", balance, "wanted", minApply)
	}
	...
	grc21Issuer.TransactOpts.Value = big.NewInt(0)
	airDropAmount := big.NewInt(1000000000)
	// airdrop token grc21 to a address no gela
	tx, err := grc21.Transfer(airdropAddr, airDropAmount)
	if err != nil {
	...
	// access to address which received token grc21 but dont have gela
	key1TransactOpts := bind.NewKeyedTransactor(airdropKey)
	key1Trc20, _ := NewGRC21(key1TransactOpts, grc21TokenAddr, contractBackend)
	...
	balance, err = grc21.BalanceOf(subAddr)
	...
	remainAirDrop = remainAirDrop.Sub(remainAirDrop, GRC21fee)
	// check balance grc21 again
	balance, err = grc21.BalanceOf(airdropAddr)
