...
contract RelayerRegistration {
    ...
    address constant private gelaNative = 0x0000000000000000000000000000000000000001;
    ...
    AbstractGELXListing private GelXListing;
    ...
    constructor (address gelxListing, uint maxRelayers, uint maxTokenList, uint minDeposit) public {
        GelXListing = AbstractGELXListing(gelxListing);
        RelayerCount = 0;
    ...
    function register(address coinbase, uint16 tradeFee, address[] memory fromTokens, address[] memory toTokens) public payable {
        ...
        // check valid tokens, token must pair with gela(x/GEL)
    ...
    function depositMore(address coinbase) public payable relayerOwnerOnly(coinbase) onlyActiveRelayer(coinbase) notForSale(coinbase) nonZeroValue {
        require(msg.value >= 1 ether, "At least 1 GEL is required for a deposit request");
    ...
    function indexOf(address[] memory gelaPair, address target) internal pure returns (bool){
        for (uint i = 0; i < gelaPair.length; i ++) {
            if (gelaPair[i] == target) {
                return true;
            }
        }
        return false;
    ...
    function validateTokens(address[] memory fromTokens, address[] memory toTokens) internal returns(bool) {
        ...
        address[] memory gelaPairs = new address[](fromTokens.length);
        address[] memory nonGelaPairs = new address[](fromTokens.length);

        for (uint i = 0; i < toTokens.length; i++) {
            bool b = GelXListing.getTokenStatus(toTokens[i]) || (toTokens[i] == gelaNative);
            b = b && (GelXListing.getTokenStatus(fromTokens[i]) || fromTokens[i] == gelaNative);
            if (!b) {
                return false;
            }
            if (toTokens[i] == gelaNative) {
                gelaPairs[countPair] = fromTokens[i];
                countPair++;
            } else {
                if (fromTokens[i] == gelaNative) {
                    gelaPairs[countPair] = toTokens[i];
                    countPair++;
                }
                nonGelaPairs[countNonPair] = toTokens[i];
                countNonPair++;
            }
        }

        for (uint j = 0; j < countNonPair; j++) {
            if (!indexOf(gelaPairs, nonGelaPairs[j])) {
                return false;
            }
        }
        return true;
    ...
    function addingTokenValidation(
        ...
        address[] memory gelaPairs = new address[](RELAYER_LIST[coinbase]._toTokens.length + 1);

        bool b = GelXListing.getTokenStatus(toToken) || (toToken == gelaNative);
        b = b && (GelXListing.getTokenStatus(fromToken) || fromToken == gelaNative);
        if (!b) {
            return false;
        }

        if (fromToken == gelaNative || toToken == gelaNative) {
            return true;
        }

        // get tokens that paired with gela
        for (uint i = 0; i < RELAYER_LIST[coinbase]._toTokens.length; i++) {
            if (RELAYER_LIST[coinbase]._toTokens[i] == gelaNative) {
                gelaPairs[countPair] = RELAYER_LIST[coinbase]._fromTokens[i];
                countPair++;
            } else {
                if (RELAYER_LIST[coinbase]._fromTokens[i] == gelaNative) {
                    gelaPairs[countPair] = RELAYER_LIST[coinbase]._toTokens[i];
                    countPair++;
                }
            }
        }
        if (!indexOf(gelaPairs, toToken)) {
            return false;
        }
        return true;
    }
