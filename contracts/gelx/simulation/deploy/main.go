...
import (
	...
	"github.com/projectgela/gela/contracts/gelx"
	"github.com/projectgela/gela/contracts/gelx/simulation"
...
func main() {
	...
	grc21IssuerAddr, grc21Issuer, err := gelx.DeployGRC21Issuer(auth, client, simulation.MinGRC21Apply)
	if err != nil {
		log.Fatal("DeployGRC21Issuer", err)
	}
	grc21Issuer.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> grc21 issuer address", grc21IssuerAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : GRC Issuer")
	time.Sleep(2 * time.Second)

	//init GELX Listing in
	auth.Nonce = big.NewInt(int64(nonce + 1))
	gelxListtingAddr, gelxListing, err := gelx.DeployGELXListing(auth, client)
	if err != nil {
		log.Fatal("DeployGELXListing", err)
	}
	gelxListing.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> gelx listing address", gelxListtingAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : gelx listing ")
	time.Sleep(2 * time.Second)

	// init Relayer Registration
	auth.Nonce = big.NewInt(int64(nonce + 2))
	relayerRegistrationAddr, relayerRegistration, err := gelx.DeployRelayerRegistration(auth, client, gelxListtingAddr, simulation.MaxRelayers, simulation.MaxTokenList, simulation.MinDeposit)
	if err != nil {
	...
	lendingRelayerRegistrationAddr, lendingRelayerRegistration, err := gelx.DeployLendingRelayerRegistration(auth, client, relayerRegistrationAddr, gelxListtingAddr)
	if err != nil {
	...
	tokenList := initGRC21(auth, client, currentNonce, simulation.TokenNameList)

	currentNonce = currentNonce + uint64(len(simulation.TokenNameList)) // init smartcontract

	applyIssuer(grc21Issuer, tokenList, currentNonce)

	currentNonce = currentNonce + uint64(len(simulation.TokenNameList))
	applyGelXListing(gelxListing, tokenList, currentNonce)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddCollateral(simulation.GELANative, simulation.CollateralDepositRate, simulation.CollateralLiquidationRate, simulation.CollateralRecallRate)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddBaseToken(simulation.GELANative)
	if err != nil {
		log.Fatal("Lending add base token GEL", err)
	}
	...
	relayerRegistration, err = gelx.NewRelayerRegistration(ownerRelayer, relayerRegistrationAddr, client)
	if err != nil {
	...
	/*
		for _, token := range tokenList {
			...
			toTokens = append(toTokens, simulation.GELANative)
		}
	*/

	// GEL/BTC
	fromTokens = append(fromTokens, simulation.GELANative)
	...
	fmt.Println("wait 2s to apply token to list gelx")
	time.Sleep(2 * time.Second)

	// Lending apply
	nonce = nonce + 1
	lendingRelayerRegistration, err = gelx.NewLendingRelayerRegistration(ownerRelayer, lendingRelayerRegistrationAddr, client)
	if err != nil {
		log.Fatal("NewRelayerRegistration", err)
	}
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	...
	// GEL 1 min
	baseTokens = append(baseTokens, simulation.GELANative)
	terms = append(terms, big.NewInt(60))
	collaterals = append(collaterals, common.HexToAddress("0x0"))
...
func initGRC21(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		tokenCap := simulation.GRC21TokenCap
		if tokenName == "ADA" {
			d = 0
			tokenCap = new(big.Int).Div(simulation.GRC21TokenCap, simulation.BaseGEL)
		}
		if tokenName == "USDT" {
			d = 6
			tokenCap = new(big.Int).Div(simulation.GRC21TokenCap, big.NewInt(1000000000000))
			withdrawFee = big.NewInt(970000)
		}
		if tokenName == "BTC" {
			d = 8
			tokenCap = new(big.Int).Div(simulation.GRC21TokenCap, big.NewInt(10000000000))
			withdrawFee = big.NewInt(40000)
		}
		if tokenName == "ETH" {
			withdrawFee = big.NewInt(3000000000000000)
		}
		tokenAddr, _, err := gelx.DeployGRC21(auth, client, simulation.Owners, simulation.Required, tokenName, tokenName, d, tokenCap, simulation.GRC21TokenFee, depositFee, withdrawFee)
		if err != nil {
			log.Fatal("DeployGRC21 ", tokenName, err)
		}
...
func applyIssuer(grc21Issuer *gelx.GRC21Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		grc21Issuer.TransactOpts.Nonce = big.NewInt(int64(nonce))
		grc21Issuer.TransactOpts.Value = simulation.MinGRC21Apply
		_, err := grc21Issuer.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("grc21Issuer Apply  ", token["name"].(string), err)
		}
...
func applyGelXListing(gelxListing *gelx.GELXListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		gelxListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		gelxListing.TransactOpts.Value = simulation.GelXListingFee
		_, err := gelxListing.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("gelxListing Apply ", token["name"].(string), err)
		}
		fmt.Println("applyGelXListing ", token["name"].(string))
		nonce = nonce + 1
	}
	time.Sleep(5 * time.Second)
}
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			grc21Contract, _ := gelx.NewGRC21(auth, token["address"].(common.Address), client)
			grc21Contract.TransactOpts.Nonce = big.NewInt(int64(nonce))
			...
			_, err := grc21Contract.Transfer(address, amount)
			if err == nil {
