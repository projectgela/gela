...
func SubRelayerFee(relayer common.Address, fee *big.Int, statedb *state.StateDB) error {
	...
	log.Debug("ApplyGelXMatchedTransaction settle balance: SubRelayerFee BEFORE", "relayer", relayer.String(), "balance", balance)
	if balance.Cmp(fee) < 0 {
		return errors.Errorf("relayer %s isn't enough gela fee", relayer.String())
	} else {
		...
		log.Debug("ApplyGelXMatchedTransaction settle balance: SubRelayerFee AFTER", "relayer", relayer.String(), "balance", balance)
		return nil
	}
}
...
func CheckRelayerFee(relayer common.Address, fee *big.Int, statedb *state.StateDB) error {
	...
	if new(big.Int).Sub(balance, fee).Cmp(new(big.Int).Mul(common.BasePrice, common.RelayerLockedFund)) < 0 {
		return errors.Errorf("relayer %s isn't enough gela fee : balance %d , fee : %d ", relayer.Hex(), balance.Uint64(), fee.Uint64())
	}
	return nil
}
func AddTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) error {
	// GEL native
	if token.String() == common.GelaNativeAddress {
		balance := statedb.GetBalance(addr)
		log.Debug("ApplyGelXMatchedTransaction settle balance: ADD TOKEN GEL NATIVE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		statedb.AddBalance(addr, value)
		balance = statedb.GetBalance(addr)
		log.Debug("ApplyGelXMatchedTransaction settle balance: ADD GEL NATIVE BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
	...
	if statedb.Exist(token) {
		...
		log.Debug("ApplyGelXMatchedTransaction settle balance: ADD TOKEN BALANCE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		balance = new(big.Int).Add(balance, value)
		statedb.SetState(token, locHash, common.BigToHash(balance))
		log.Debug("ApplyGelXMatchedTransaction settle balance: ADD TOKEN BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		return nil
	} else {
...
func SubTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB) error {
	...
	if token.String() == common.GelaNativeAddress {
		...
		balance := statedb.GetBalance(addr)
		log.Debug("ApplyGelXMatchedTransaction settle balance: SUB GEL NATIVE BALANCE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		...
		balance = statedb.GetBalance(addr)
		log.Debug("ApplyGelXMatchedTransaction settle balance: SUB GEL NATIVE BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		return nil
	}
	...
	if statedb.Exist(token) {
		...
		locHash := common.BigToHash(GetLocMappingAtKey(addr.Hash(), slot))
		balance := statedb.GetState(token, locHash).Big()
		log.Debug("ApplyGelXMatchedTransaction settle balance: SUB TOKEN BALANCE BEFORE", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		if balance.Cmp(value) < 0 {
			return errors.Errorf("value %s in token %s not enough , have : %s , want : %s  ", addr.String(), token.String(), balance, value)
		}
		balance = new(big.Int).Sub(balance, value)
		statedb.SetState(token, locHash, common.BigToHash(balance))
		log.Debug("ApplyGelXMatchedTransaction settle balance: SUB TOKEN BALANCE AFTER", "token", token.String(), "address", addr.String(), "balance", balance, "orderValue", value)
		return nil
	} else {
		return errors.Errorf("token %s isn't exist", token.String())
	}
...
func CheckSubTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB, mapBalances map[common.Address]map[common.Address]*big.Int) (*big.Int, error) {
	...
	if token.String() == common.GelaNativeAddress {
		...
		log.Debug("CheckSubTokenBalance settle balance: SUB GEL NATIVE BALANCE ", "token", token.String(), "address", addr.String(), "balance", balance, "value", value, "newBalance", newBalance)
...
func CheckAddTokenBalance(addr common.Address, value *big.Int, token common.Address, statedb *state.StateDB, mapBalances map[common.Address]map[common.Address]*big.Int) (*big.Int, error) {
	...
	if token.String() == common.GelaNativeAddress {
		...
		log.Debug("CheckAddTokenBalance settle balance: ADD GEL NATIVE BALANCE ", "token", token.String(), "address", addr.String(), "balance", balance, "value", value, "newBalance", newBalance)
		return newBalance, nil
	}
	// GRC tokens
	if statedb.Exist(token) {
...
func CheckSubRelayerFee(relayer common.Address, fee *big.Int, statedb *state.StateDB, mapBalances map[common.Address]*big.Int) (*big.Int, error) {
	...
	if balance.Cmp(fee) < 0 {
		return nil, errors.Errorf("relayer %s isn't enough gela fee", relayer.String())
	} else {
