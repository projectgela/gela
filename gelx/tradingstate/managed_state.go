...
type GelXManagedState struct {
	*TradingStateDB
	mu        sync.RWMutex
	exchanges map[common.Hash]*exchanges
}

// GelXManagedState returns a new managed state with the statedb as it's backing layer
func ManageState(statedb *TradingStateDB) *GelXManagedState {
	return &GelXManagedState{
...
func (ms *GelXManagedState) SetState(statedb *TradingStateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *GelXManagedState) RemoveNonce(addr common.Hash, n uint64) {
...
func (ms *GelXManagedState) NewNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *GelXManagedState) GetNonce(addr common.Hash) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	if ms.hasAccount(addr) {
		account := ms.getAccount(addr)
...
func (ms *GelXManagedState) SetNonce(addr common.Hash, nonce uint64) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *GelXManagedState) HasAccount(addr common.Hash) bool {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *GelXManagedState) getAccount(addr common.Hash) *exchanges {
