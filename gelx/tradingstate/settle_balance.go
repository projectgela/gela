...
const DefaultFeeRate = 10 // 10 / GelXBaseFee = 10 / 10000 = 0.1%
...
func GetSettleBalance(quotePrice *big.Int, takerSide string, takerFeeRate *big.Int, baseToken, quoteToken common.Address, makerPrice *big.Int, makerFeeRate *big.Int, baseTokenDecimal *big.Int, quoteTokenDecimal *big.Int, quantityToTrade *big.Int) (*SettleBalance, error) {
	...
	if takerSide == Bid {
		...
		if quoteToken.String() != common.GelaNativeAddress && quotePrice != nil && quotePrice.Cmp(common.Big0) > 0 {
			// defaultFeeInGELA
			defaultFeeInGELA := new(big.Int).Mul(defaultFee, quotePrice)
			defaultFeeInGELA = new(big.Int).Div(defaultFeeInGELA, quoteTokenDecimal)
			...
			if (exMakerReceivedFee.Cmp(common.RelayerFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInGELA.Cmp(common.RelayerFee) <= 0 {
				log.Debug("makerFee too small", "quoteTokenQuantity", quoteTokenQuantity, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "quotePrice", quotePrice, "defaultFeeInGELA", defaultFeeInGELA)
				return result, ErrQuantityTradeTooSmall
			}
			...
			if (exTakerReceivedFee.Cmp(common.RelayerFee) <= 0 && exTakerReceivedFee.Sign() > 0) || defaultFeeInGELA.Cmp(common.RelayerFee) <= 0 {
				log.Debug("takerFee too small", "quoteTokenQuantity", quoteTokenQuantity, "takerFee", takerFee, "exTakerReceivedFee", exTakerReceivedFee, "quotePrice", quotePrice, "defaultFeeInGELA", defaultFeeInGELA)
				return result, ErrQuantityTradeTooSmall
			}
		} else if quoteToken.String() == common.GelaNativeAddress {
		...
		if quoteToken.String() != common.GelaNativeAddress && quotePrice != nil && quotePrice.Cmp(common.Big0) > 0 {
			...
			if (exMakerReceivedFee.Cmp(common.RelayerFee) <= 0 && exMakerReceivedFee.Sign() > 0) || defaultFeeInGELA.Cmp(common.RelayerFee) <= 0 {
				log.Debug("makerFee too small", "quoteTokenQuantity", quoteTokenQuantity, "makerFee", makerFee, "exMakerReceivedFee", exMakerReceivedFee, "quotePrice", quotePrice, "defaultMakerFeeInGELA", defaultFeeInGELA)
				return result, ErrQuantityTradeTooSmall
			}
			exTakerReceivedFee := new(big.Int).Mul(takerFee, quotePrice)
			exTakerReceivedFee = new(big.Int).Div(exTakerReceivedFee, quoteTokenDecimal)
