	...
	for {
		...
		case ev := <-self.txCh:
			...
			if atomic.LoadInt32(&self.mining) == 0 {
				...
				feeCapacity := state.GetGRC21FeeCapacityFromState(self.current.state)
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	var gelxState *tradingstate.TradingStateDB
	var lendingState *lendingstate.LendingStateDB
	if self.config.Posv != nil {
		gelX := self.eth.GetGelX()
		gelxState, err = gelX.GetTradingState(parent, author)
		if err != nil {
			log.Error("Failed to get gelx state ", "number", parent.Number(), "err", err)
			return err
		}
		lending := self.eth.GetGelXLending()
...
func (self *worker) commitNewWork() {
	...
	feeCapacity := state.GetGRC21FeeCapacityFromStateWithCache(parent.Root(), work.state)
	...
	if atomic.LoadInt32(&self.mining) == 1 {
		...
		if self.config.Posv != nil && self.chain.Config().IsTIPGelX(header.Number) {
			gelX := self.eth.GetGelX()
			gelXLending := self.eth.GetGelXLending()
			if gelX != nil && header.Number.Uint64() > self.config.Posv.Epoch {
				if header.Number.Uint64()%self.config.Posv.Epoch == 0 {
					err := gelX.UpdateMediumPriceBeforeEpoch(header.Number.Uint64()/self.config.Posv.Epoch, work.tradingState, work.state)
					if err != nil {
				...
				if header.Number.Uint64()%self.config.Posv.Epoch != 0 {
					...
					tradingTxMatches, tradingMatchingResults = gelX.ProcessOrderPending(self.coinbase, self.chain, tradingOrderPending, work.state, work.tradingState)
					...
					lendingInput, lendingMatchingResults = gelXLending.ProcessOrderPending(header, self.coinbase, self.chain, lendingOrderPending, work.state, work.lendingState, work.tradingState)
					...
					if header.Number.Uint64()%self.config.Posv.Epoch == common.LiquidateLendingTradeBlock {
						updatedTrades, liquidatedTrades, autoRepayTrades, autoTopUpTrades, autoRecallTrades, err = gelXLending.ProcessLiquidationData(header, self.chain, work.state, work.tradingState, work.lendingState)
						if err != nil {
				...
				if len(tradingTxMatches) > 0 {
					...
					tx := types.NewTransaction(nonce, common.HexToAddress(common.GelXAddr), big.NewInt(0), txMatchGasLimit, big.NewInt(0), txMatchBytes)
				...
				if len(lendingInput) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					lendingTx := types.NewTransaction(nonce, common.HexToAddress(common.GelXLendingAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), lendingDataBytes)
				...
				if len(updatedTrades) > 0 {
					...
					nonce := work.state.GetNonce(self.coinbase)
					finalizedTx := types.NewTransaction(nonce, common.HexToAddress(common.GelXLendingFinalizedTradeAddress), big.NewInt(0), txMatchGasLimit, big.NewInt(0), finalizedTradeData)
		...
		GelxStateRoot := work.tradingState.IntermediateRoot()
		LendingStateRoot := work.lendingState.IntermediateRoot()
		txData := append(GelxStateRoot.Bytes(), LendingStateRoot.Bytes()...)
...
func (env *Work) commitTransactions(mux *event.TypeMux, balanceFee map[common.Address]*big.Int, txs *types.TransactionsByPriceAndNonce, specialTxs types.Transactions, bc *core.BlockChain, coinbase common.Address) {
	...
	for _, tx := range specialTxs {
		...
		if tx.IsGelaZApplyTransaction() {
			...
			if err := core.ValidateGelaZApplyTransaction(bc, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				log.Debug("GelaZApply: invalid token", "token", common.BytesToAddress(tx.Data()[4:]).Hex())
				txs.Pop()
				continue
			}
		}
		...
		if tx.IsGelXApplyTransaction() {
			copyState, _ := bc.State()
			if err := core.ValidateGelXApplyTransaction(bc, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
				log.Debug("GelXApply: invalid token", "token", common.BytesToAddress(tx.Data()[4:]).Hex())
				txs.Pop()
				continue
			}
		}
		...
		if tokenFeeUsed {
			...
			if env.header.Number.Cmp(common.TIPGRC21Fee) > 0 {
	...
	state.UpdateGRC21Fee(env.state, balanceUpdated, totalFeeUsed)
