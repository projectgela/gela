...
import (
	...
	"github.com/projectgela/gela/internal/ethapi"
...
func New(ctx *node.ServiceContext, config *Config, gelXServ *gelx.GelX, lendingServ *gelxlending.Lending) (*Ethereum, error) {
	...
	// Inject GelX Service into main Eth Service.
	if gelXServ != nil {
		eth.GelX = gelXServ
	}
	if lendingServ != nil {
	...
	if eth.chainConfig.Posv != nil {
		...
		c.GetGelXService = func() posv.TradingService {
			return eth.GelX
		}
	...
	eth.blockchain, err = core.NewBlockChainEx(chainDb, gelXServ.GetLevelDB(), cacheConfig, eth.chainConfig, eth.engine, vmConfig)
	if err != nil {
		return nil, err
	}
...
func (s *Ethereum) GetGelX() *gelx.GelX {
	return s.GelX
}
...
func (s *Ethereum) GetGelXLending() *gelxlending.Lending {
	return s.Lending
}
