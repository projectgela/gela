...
func VerifyBalance(isGelXLendingFork bool, statedb *state.StateDB, lendingStateDb *LendingStateDB,
	orderType, side, status string, userAddress, relayer, lendingToken, collateralToken common.Address,
	quantity, lendingTokenDecimal, collateralTokenDecimal, lendTokenGELAPrice, collateralPrice *big.Int,
	...
	case Market, Limit:
		...
		case Investing:
			...
			case LendingStatusNew:
				...
				if lendTokenGELAPrice != nil && lendTokenGELAPrice.Sign() > 0 {
					...
					defaultFeeInGELA := common.Big0
					if lendingToken.String() != common.GelaNativeAddress {
						defaultFeeInGELA = new(big.Int).Mul(defaultFee, lendTokenGELAPrice)
						defaultFeeInGELA = new(big.Int).Div(defaultFeeInGELA, lendingTokenDecimal)
					} else {
						defaultFeeInGELA = defaultFee
					}
					if defaultFeeInGELA.Cmp(common.RelayerLendingFee) <= 0 {
		...
		case Borrowing:
			...
			case LendingStatusNew:
				...
				settleBalanceResult, err := GetSettleBalance(isGelXLendingFork, Borrowing, lendTokenGELAPrice, collateralPrice, depositRate, borrowingFeeRate, lendingToken, collateralToken, lendingTokenDecimal, collateralTokenDecimal, quantity)
				if err != nil {
					return err
				}
