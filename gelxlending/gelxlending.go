...
import (
	...
	gelx "github.com/projectgela/gela/gelx"
	"github.com/projectgela/gela/gelx/tradingstate"
	gelxDAO "github.com/projectgela/gela/gelxDAO"
...
func New(gelx *gelx.GelX) *Lending {
	...
	lending.StateCache = lendingstate.NewDatabase(gelx.GetLevelDB())
	lending.gelx = gelx
	return lending
}

func (l *Lending) GetLevelDB() gelxDAO.GelXDAO {
	return l.gelx.GetLevelDB()
}

func (l *Lending) GetMongoDB() gelxDAO.GelXDAO {
	return l.gelx.GetMongoDB()
}
	...
	return []rpc.API{
		{
			...
			Service:   NewPublicGelXLendingAPI(l),
			Public:    true,
		},
	}
}
...
func (l *Lending) GetLendingState(block *types.Block, author common.Address) (*lendingstate.LendingStateDB, error) {
	...
	if l.StateCache == nil {
		return nil, errors.New("Not initialized gelx")
	}
...
func (l *Lending) RollbackLendingData(txhash common.Hash) error {
	...
	if items != nil {
		for _, item := range items.([]*lendingstate.LendingItem) {
			c, ok := l.lendingItemHistory.Get(txhash)
			log.Debug("gelxlending reorg: rollback lendingItem", "txhash", txhash.Hex(), "item", lendingstate.ToJSON(item), "lendingItemHistory", c)
			if !ok {
				log.Debug("gelxlending reorg: remove item due to no lendingItemHistory", "item", lendingstate.ToJSON(item))
			...
			if (lendingItemHistory == lendingstate.LendingItemHistoryItem{}) {
				log.Debug("gelxlending reorg: remove item due to empty lendingItemHistory", "item", lendingstate.ToJSON(item))
				if err := db.DeleteObject(item.Hash, &lendingstate.LendingItem{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingItem. Err: %v . Item: %s", err.Error(), lendingstate.ToJSON(item))
				}
				continue
			...
			log.Debug("gelxlending reorg: update item to the last lendingItemHistory", "item", lendingstate.ToJSON(item), "lendingItemHistory", lendingItemHistory)
	...
	if items != nil {
		for _, trade := range items.([]*lendingstate.LendingTrade) {
			c, ok := l.lendingTradeHistory.Get(txhash)
			log.Debug("gelxlending reorg: rollback LendingTrade", "txhash", txhash.Hex(), "trade", lendingstate.ToJSON(trade), "LendingTradeHistory", c)
			if !ok {
				log.Debug("gelxlending reorg: remove trade due to no LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
			...
			if (lendingTradeHistoryItem == lendingstate.LendingTradeHistoryItem{}) {
				log.Debug("gelxlending reorg: remove trade due to empty LendingTradeHistory", "trade", lendingstate.ToJSON(trade))
				if err := db.DeleteObject(trade.Hash, &lendingstate.LendingTrade{}); err != nil {
					return fmt.Errorf("failed to remove reorg LendingTrade. Err: %v . Trade: %s", err.Error(), lendingstate.ToJSON(trade))
				}
				continue
			...
			log.Debug("gelxlending reorg: update trade to the last lendingTradeHistoryItem", "trade", lendingstate.ToJSON(trade), "lendingTradeHistoryItem", lendingTradeHistoryItem)
			if err := db.PutObject(trade.Hash, trade); err != nil {
