...
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	if tx.IsGelaZApplyTransaction() {
		...
		if err := core.ValidateGelaZApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
	...
	if tx.IsGelXApplyTransaction() {
		copyState := pool.currentState(ctx).Copy()
		if err := core.ValidateGelXApplyTransaction(pool.chain, nil, copyState, common.BytesToAddress(tx.Data()[4:])); err != nil {
			return err
		}
	}
